# RFD 1049: A billboard gallery of the RFD 1040 dataset, in one USD stage

**State:** prediscussion
**Scope:** RFD 1040's dataset, `apps/usd_viewer_app/`

## Problem

RFD 1040's dataset holds about 15,000 anime images and captions, on
...
RFD 1040 gives the dataset this RFD displays. RFD 1035 makes USD the
...
RFD 103e gives the Fly.io toplevel this RFD's app first deployed
beside. RFD 104c moves that companion app to its own deploy target
entirely (`apps/usd_viewer_app/`), reached through a
`GallerySource` port, and gives the `usd-viewer` patch's real,
current form. RFD 104d decides Tigris, not `versitygw`, as the
