# RFD 0083 details: coordinate system, requirements, failure codes, and the Blender path
...
| --- | --- | --- |
| VRM load | A `.vrm` file, loot assets, and so on | `vrmLoader.normalizeVRM` only; no contract flags, no `preserveExportedOrientation`. RFD 0104 gives the full pipeline. |
