...
| **bnb NF4 4-bit** as a QAFT / QAT path                          | ships as base+adapter not a single 4-bit checkpoint; slow-kernel fallback on non-64-aligned shapes (OmniGen2's 2520); GPTQ/AWQ deliver real 4-bit QAFT — see below |
| **Post-quantization fine-tuning** (quantize-first, adapt-after) | trains an adapter that never sees quantization; ships as two files at two precisions. Fine-tune unquantized then quantize (GPTQ/AWQ/HQQ) — see below               |
