...
with no appearance is a different thing from absent geometry, and RFD 1121 means the first.

## OpenUSD is where the topology now lives

RFD 1053 makes OpenUSD the internal format. `export_hm08_usd.py` in
...
`strength` alone. RFD 1121 requires a depth control wherever generated geometry must match an
...
Note also RFD 1043's open question. No measurement compares Q4_K_M against bf16 for this model,
...
earlier version of this note said RFD 1126 retracted half of it, because that RFD replaced
the fit with a learned student and a student is an inverse model. RFD 1126 now unrolls the
...
negative controls that each must fail. RFD 1121 records the full measurement.
...
**Hailo-first reaches backwards into the training run.** RFD 1126 already decided the backbone
