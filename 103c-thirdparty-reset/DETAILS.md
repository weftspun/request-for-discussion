# RFD 103c details: the second move, why a real move, what stays, paths, verification
...
`thirdparty/3d_studio/` through a port, per RFD 1017, or it does not
touch it.

## Why a real move, not a rewrite of every import

RFD 1039 says a sweep is the wrong shape for a large, low-value
change. "Convert one when its model is next worked on, and not in a
sweep," in RFD 1039's own words. RFD 1017 says the same rule for
`src/library/`. "A module moves when a port covers it, and not
before."

Neither RFD 1017's per-file rule, nor RFD 1039's no-sweep rule,
...
**`.devcontainer/`** — RFD 1038's dev container, for the Elixir
...
and write that tree, not this one. RFD 103f later deleted
...
`weftspun_studio/` prefix RFD 103a wrote. `scripts/deploy-weftspun-quadlet.sh`
...
actually in. RFD 103f later deleted that script and that npm entry.
...
swap. RFD 1039 records the later fix. All 105 tests pass now.
...
pre-existing `glbCompress.js` export failures RFD 1017 already
records, and no import-resolution failure.

## What this does not do

It does not fix the `prepareGlbForApiUpload` gap RFD 1017 already
...
It does not touch RFD 1017's `src/core/` / `src/chain/` split. Both
