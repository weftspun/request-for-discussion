# RFD 0086 details: roles, log fields, ownership, and the sync commands
...
- The NVIDIA XR AI stack runs on the DGX only (`/home/sifr/xr-ai`); RFD 0095 gives its own Media Hub access path.
...
NVIDIA Sync), both as user `sifr`. RFD 0101 gives the full naming
rule.
...
RFD 0088 gives the HTTPS certificate setup, for Galaxy XR access.
RFD 0105 gives webcam avatar control's `webcamDebug` and
remote-logging flags. RFD 0082 gives the Android Studio AI brief,
APK relay, and headset test steps. RFD 0096 gives the OpenXR
