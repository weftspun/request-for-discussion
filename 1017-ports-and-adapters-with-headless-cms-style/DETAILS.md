# RFD 1017 details: the layout, the content split, the asset port, method, risks, status
...
The composition root sits outside `src/core/`, because it must know
every side, and the core may not reach a chain.
...
| ----------------------------- | -------- | ------------------------- |
| Which models exist            | Yes      | `CatalogSource`, RFD 1016 |
...
Red, green, refactor, as RFD 1016 sets out. The RFD is the first red.
...
predate the rebrand, and RFD 1016 records them. The Status section
...
- `src/components/TaskManager.jsx` line 1163 imports
  `prepareGlbForApiUpload` from `glbCompress.js`. RFD 103d later
  implemented that function as an interim stopgap.
