# RFD 104b details: registration steps and the login flow
...
6. Both values become Fly secrets, `GITHUB_OAUTH_CLIENT_ID` and
   ...
   same pattern RFD 1049's `VGW_ACCESS_KEY`/`VGW_SECRET_KEY` already
...
2. A signed-cookie session needs its own signing key, separate from
   ...
   same urandom-based pattern RFD 1049 already used for
