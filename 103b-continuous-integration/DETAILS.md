# RFD 103b details: the steps, why one script, what it does not cover
...
3. `podman build` (or `docker build`, whichever is on `PATH`) for
   both RFD 103a images. No push, build only.
...
because this project develops against Podman (RFD 103a), while
...
RFD 103a's `deploy-weftspun-quadlet.sh` stays outside it too.
Continuous integration is not continuous deployment, and RFD 103a's
