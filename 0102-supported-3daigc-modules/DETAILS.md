# RFD 0102 details: the task table, splat status, architecture, and further reading
...
fabric through the MSF Map Service (RFD 0100); Kimodo
...
path before it can publish to OMB at all (RFD 0100 gives why).
...
and `viewer` reference spaces (RFD 0108 gives the implementation);
WebXR expression tracking when the browser exposes it, a native face
relay through a companion APK when it does not (RFD 0096, RFD
0082).
...
RFD 0094 (multi-image routing), RFD 0095 (NVIDIA XR AI), RFD 0086
(dev machine topology), RFD 0107 (world package format), RFD 0100
(spatial fabric/RP1), RFD 0084 (avatar pipeline, client side), RFD
0090 (IWSDK integration), RFD 0096 (OpenXR face tracking), RFD 0082
(the Android XR face-bridge APK), RFD 0105 (webcam/avatar control),
RFD 0108 (XR floor anchoring and backgrounds), RFD 0088 (HTTPS
setup), RFD 0085 (the code map).
