# RFD 0121 details: the coverage table, the rebuild pipeline, and the ledger protocol
...
6. **Edit one category under a mask.** VoxHammer adds or replaces one tag. Its splice step
   runs before the decode, so geometry outside the mask does not move. RFD 0048 states this
   guard.
