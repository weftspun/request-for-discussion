# RFD 1078 details: path changes, the new deploy convention, and open work
...
new repo's history starts at RFD 104c's move into `apps/`, not the
project's full history; the fuller history stays in
`weftspun-3d-studio`'s own git log.

## The `/opt/weftspun/<repo>/` convention

The self-hosted Quadlet deploy (RFD 103a) used to `rsync` one
...
- `scripts/push_gallery_to_vgw.exs`: pushed proof assets to
  versitygw's S3 API. Already dead before this split, since RFD 104f
  removed versitygw.
