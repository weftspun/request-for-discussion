...
CLAUDE.md's hard constraint is "the local desktop GPU is the only
compute; RunPod and Vast.ai blocklisted". Five open RFDs still name
rented compute as their decision -- the RFD's whole reason for
existing contradicts the blocklist.

## Decision

Abandon each of these RFDs. They move to `abandoned` alongside this
document landing. The RFDs themselves stay in the register for
citation, per CLAUDE.md's "retractions stay in place next to what
they retract".
...
Anchor: CLAUDE.md's Compute hard constraint and the RunPod, Vast.ai
blocklist rows.
