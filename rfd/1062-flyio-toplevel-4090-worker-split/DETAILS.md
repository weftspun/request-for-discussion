# RFD 1062 details: why Fly, why Tailscale, the existing port, what ships, Gall's law, scope
...
market RFD 1055 already priced vast.ai against. Moving the GPU there
...
The router, the planner, and CockroachDB need none of that. RFD 1027
...
RFD 1058 grounds its whole design in one fact. A bound port past
...
This box already runs Tailscale. RFD 1058's own firewall
...
That is a third bind mode past the two RFD 1058 already uses. It is
...
RFD 1023 designed `Ports.JobSink` / `Ports.JobSource` so the router
...
into RFD 1003's four. A worker adapter for this box does the same
...
- `Dockerfile` at the repo root (RFD 1060 moved it there, and RFD
  1058 already proved it builds a working release image) and a
  `fly.toml` this RFD does not yet write.
- CockroachDB, co-located with the router the same way RFD 1058 puts
...
- The built browser client (`thirdparty/3d_studio/`), served the way
  RFD 1019 already intends: the router as the one thing a browser
  talks to.

**Worker, staying local:**

- `weftspun.network`, and the quadlet isolation pattern RFD 1058
...
RFD 1055 already invokes this: rent a card only after the owned one
...
Renting on vast.ai stays exactly where RFD 1055 leaves it, as the
...
bearer-token auth already wired in. RFD 1061 names the same pair,
...
a `.caibx` URL as the asset reference, the same shape RFD 1061 needs
for browser uploads.

Unlike RFD 1061, this adapter needs no NIF. RFD 1061's gap is
...
job-control envelope. `Ports.JobSource` already defines RFD 1003's
...
A third process, `versitygw`, runs colocated too, per RFD 1073 and
RFD 1058's own zero-trust rule, bound to `127.0.0.1:10000`.
...
move CockroachDB off this box's own RFD 1058 deployment, since the
Fly toplevel runs its own separate, colocated CockroachDB instead.
RFD 1057 tracks the rest as open work.

It does not retire `ReplicateJobs`. RFD 1055 already records why that
