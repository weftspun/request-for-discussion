# RFD 1037 details: the local worker, the rented tier priced for later, the two phases, image shape, unresolved

## What was considered, and when it applies

This project owns an RTX 4090, in this box. RFD 103e's Gall's law
...
1. Commit the CockroachDB fixes and the let-it-crash pass. Done in
   ...
   RFD 1038 records why.
2. Delete `cms/`, and fold the planning documents, the `Planner` port,
   and `TaskweftPlanner` into `weftspun_studio`. Done in `c2d659f7`.
   That settles the overlap RFD 1017 and RFD 1036 carried.
3. Remove the Replicate passthrough and `ReplicateJobs`. Open, and see
   below.
4. Abandon the Cog build. Done in `06c5c4ba`. RFD 1024 selects plain
   Docker, and RFD 1028 records a contract stage tested in Docker.
...
dependency on Replicate is acceptable, matching the RFD 101c gate's
...
adapter changes host and keeps its port, which is what RFD 1017
makes possible.
...
2. Run it on this box's own RTX 4090, through RFD 103a's Quadlets,
...
weights. RFD 1024 keeps one model per image.

The two disagree, and the weights decide it. RFD 101a sums the catalog
