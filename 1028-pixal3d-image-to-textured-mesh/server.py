"""Pixal3D image to textured mesh, PBR. RFD 1028.

An HTTP server in a plain Docker image. RFD 1024 records why this is
not a Cog.
...
RFD 1035 makes OpenUSD the internal format, thus a result carries a
...
# and no 24 GB download. RFD 1028 records that test.
...
def _to_usd(glb: Path, work: Path) -> Path:
    ...
    RFD 1035 makes USD the internal format. This model authors the base
