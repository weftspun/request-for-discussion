defmodule ProseTropes do
  @moduledoc """
  Gate: the aphoristic negative definition, ratcheted toward how people
  actually write.

  ## The trope

  "A number nobody waited for isn't a measurement." "An ignore is not a
  lock." A short sentence, generic subject, predicate a negation, landing as
  a maxim. tropes.fyi catalogues its neighbours — negative parallelism
  ("It's not X, it's Y") and quotable one-liners — and stops short of this
  form, so the gate names it.

  ## Calibrated against the commons rather than against ourselves

  A first version took its ceiling from the p90 of this repository's own
  files, which bakes the habit in: a document may be as aphoristic as its
  neighbours, and its neighbours are us. Measured instead against
  ...
  Eight times the commons rate, in prose measurably harder to read. Public
  domain rather than CC-BY-SA, because share-alike is blocklisted here.
  Gutenberg ids 11, 55, 21, 74 and 16, fetched 2026-08-23 and recorded here
  rather than re-fetched: a gate needing the network fails for reasons
  unrelated to the file under test.

  Genre explains part of the gap. Strunk's Elements of Style, prescriptive
  writing guidance like ours, scores 1.01 per 1000 at Flesch 58.7 — three
  times the narrative rate. Some of our density comes from the genre. Three
  times the commons rate is 0.90 against our 2.69, which leaves most of the
  gap to the habit.

  Not obtained, named rather than dropped: pre-2020 US government
  plain-language guidance, which is public domain and written to be read.
  plainlanguage.gov serves guidelines whose text extracts to 413 words, and
  the 2011 PDF URL returns HTML.

  ## The ratchet

  The tree walks down instead of being condemned:
  ...
  The other gate in this workspace is `Mix.Tasks.CheckSkipTags`, and this
  follows its shape — documented switches, an exit code, controls that must
  fail. A standalone script rather than a Mix task because this repository is
  prose and has no mix project; `elixir scripts/check_prose_tropes.exs`
  needs nothing built.

  ## Equivalence with the Python it replaces

  Measured rather than assumed. Both were run over the same six files and
  agreed to two decimals every time: 2.66, 0.90, 0.00, 2.23, 0.83, 0.82,
  with identical control output. `--rate <file>` prints one file's figure,
  which is how that comparison was made and how the next one can be. The
  Python is deleted, so the rule has one home.
  ...
  `@maxim` catches a short sentence whose predicate is a negation. Length
  does real work: an aphorism is pithy, and a long negating sentence is
  usually an argument.

  Use-not-mention: quoted spans are dropped before scanning. A gate that
  defines a trope has to quote specimens of it, and a control that
  reintroduces a defect has to contain the defect, so counting those would
  fail every file for saying what it catches.

  USD carries prose that this gate never scored. `rfd1122-plan.usda` holds
  11,096 words of `doc` strings — the largest single body of prose in this
  repository — at 2.16 per 1k, higher than CLAUDE.md or BLOCKLIST.md. It
  went unscored because this gate selected markdown and source, and a layer
  file is neither. Only the doc strings are read: scoring the whole file
  would count `float2 extent = (0, 1)` as prose, and the quote-stripping
  rule would eat a single-line `doc = "..."` entirely.
  """
  ...
  @doc """
  Controls. The detector and the decision are separate code and fail
  separately: all four detector controls can pass while the ratchet returns
  0 on a file that got worse.
  """
  def self_test do
    ...
    # A fixture written with escaped quotes would split at the escape and leak
    # its specimen out of the quote-strip. Build the quote character from a
    # variable instead.
    q = <<34>>
