defmodule HouseholdUnits do
  ...
  CLAUDE.md, under "How measurements are reported", says to pair every
  physical measurement with a household object, because "4.3 mm" does not
  tell a reader whether an error matters and "about three stacked pennies"
  does. It also says that where a script prints measurements repeatedly, it
  should carry a helper rather than rely on recall.

  Recall duly failed. Across one working session the rule was broken at least
  three times in prose that reported a pixel error, a quantisation step of
  about a sixtieth of a credit card, and a stride gap of some five soda cans,
  each time bare, by an author who had read the rule that morning. A habit
  that survives knowing about it needs a gate.

  ## What counts as paired

  A number with a length unit is paired when a household anchor appears
  within the same sentence or the one after it. The anchors are CLAUDE.md's
  own list, which is the point: the gate reads the vocabulary from the
  agreements rather than keeping a second copy that can drift.
  ...
  Sizes in bytes, durations, counts, versions, money, and coordinates in
  pixels unless a physical unit is attached. The rule is about physical
  extent, and widening it to every number would make the gate noise and get
  it switched off.

  ## Detection floor

  An anchor phrased in words the list does not contain — "the width of a
  hair", "a grain of rice" — reads as unpaired here. That is a false alarm
  rather than a miss, which is the safer direction for a gate to be wrong
  in, and adding the phrase to CLAUDE.md fixes it in both places at once.
  ...
  ## Regex assembly notes

  `@measure` uses case-sensitive short units. The first version ignored case
  and read `$1M` of revenue as one metre. Lowercase m, mm and cm only; the
  spelled-out words stay case-insensitive because a sentence may begin with
  one. A leading currency symbol is rejected outright.

  `@sentence` treats a period as a stop only when it has no digit on both
  sides. Splitting on every period cut "0.02 m" into "02 m" and reported the
  fragment as unpaired while the anchor sat in the half that was thrown away.

  Use-not-mention: quoted spans are dropped before scanning. A gate for
  unpaired measurements must contain unpaired measurements — its controls
  are specimens, and its documentation quotes the anchor table.

  Table folding: contiguous pipe-led lines are joined so a header travels
  with its rows. Scoring rows separately flagged five paired measurements
  and pushed the author into rewriting a good table as prose.
  """
  ...
  @doc """
  Controls. A gate that cannot fail certifies the habit, and this one has two
  halves that fail apart: finding a measurement, and deciding whether it was
  paired.
  """
  def self_test do
    cases = [
