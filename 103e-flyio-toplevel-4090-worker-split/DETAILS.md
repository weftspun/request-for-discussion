# RFD 103e details: why Fly, why Tailscale, the existing port, what ships, Gall's law, scope
...
market RFD 1037 already priced vast.ai against. Moving the GPU there
...
The router, the planner, and CockroachDB need none of that. RFD 101b
...
RFD 103a grounds its whole design in one fact. A bound port past
...
This box already runs Tailscale. RFD 103a's own firewall
...
That is a third bind mode past the two RFD 103a already uses. It is
...
RFD 1017 designed `Ports.JobSink` / `Ports.JobSource` so the router
...
- The built browser client (`thirdparty/3d_studio/`), served the way
  RFD 1013 already intends: the router as the one thing a browser
  talks to.
...
RFD 1037 already invokes this: rent a card only after the owned one
...
Renting on vast.ai stays exactly where RFD 1037 leaves it, as the
...
bearer-token auth already wired in. RFD 103d names the same pair,
...
a `.caibx` URL as the asset reference, the same shape RFD 103d needs
for browser uploads.

Unlike RFD 103d, this adapter needs no NIF. RFD 103d's gap is
...
A third process, `versitygw`, runs colocated too, per RFD 1049 and
RFD 103a's own zero-trust rule, bound to `127.0.0.1:10000`.
...
move CockroachDB off this box's own RFD 103a deployment, since the
Fly toplevel runs its own separate, colocated CockroachDB instead.
RFD 1039 tracks the rest as open work.

It does not retire `ReplicateJobs`. RFD 1037 already records why that
